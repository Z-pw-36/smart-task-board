...
import { actionLabels, runNodeAction, runTaskAction, type TaskLifecycleAction } from "../api/taskActions";
...
export function TaskDetailPage() {
  ...
  const mutation = useMutation({
    mutationFn: async ({ action, nodeId }: { action: AllowedAction; nodeId?: string }) => {
      ...
      if (nodeId) {
        ...
        if (!isNodeExecutionAction(action)) throw new Error("该节点动作需要在验收返工区域处理。");
        await runNodeAction(taskId, nodeId, action, version, progress);
      } else {
        if (!isTaskLifecycleAction(action)) throw new Error("该任务动作需要填写完整的业务信息。");
        let reason: string | undefined;
        if (action === "return") {
          reason = window.prompt("请填写退回原因")?.trim();
          if (!reason) throw new Error("退回必须填写原因。");
        }
        await runTaskAction(taskId, action, version, reason);
      }
    },
    onSuccess: async () => {
      setNotice("操作成功。");
  ...
  if (detail.isError || actions.isError) return <ErrorState error={detail.error || actions.error} retry={() => void Promise.all([detail.refetch(), actions.refetch()])} />;
  ...
  return (
    <div className="page-stack narrow-page">
      ...
      <section className="detail-panel"><h2>任务概况</h2><dl className="detail-grid"><div><dt>创建人</dt><dd>{task.creator_employee_no}</dd></div><div><dt>主承办人</dt><dd>{task.main_assignee_employee_no || "未指定"}</dd></div><div><dt>验收人</dt><dd>{task.reviewer_employee_no || task.creator_employee_no}</dd></div><div><dt>截止</dt><dd>{formatDate(task.deadline)}</dd></div><div><dt>预计工时</dt><dd>{task.estimated_hours || "未设置"}</dd></div><div><dt>交付物</dt><dd>{task.deliverable || "未设置"}</dd></div></dl><div className="long-text"><strong>验收标准</strong><p>{task.acceptance_criteria || "未设置"}</p></div><div className="action-row">{actions.data.allowed_actions.filter(isTaskLifecycleAction).map((action) => <button className="button primary" disabled={mutation.isPending} type="button" key={action} onClick={() => mutation.mutate({ action })}>{actionLabels[action]}</button>)}</div></section>
      <section><div className="section-heading"><h2>执行节点</h2><span>{task.nodes.length} 个</span></div><div className="node-list">{task.nodes.map((node) => <article className="node-card" key={node.node_id}><div className="node-order">{node.node_order}</div><div className="node-content"><h3>{node.node_name}</h3><p>{node.action_detail || "暂无动作说明"}</p><div className="progress-track" aria-label={`进度 ${node.progress_percent}%`}><span style={{ width: `${node.progress_percent}%` }} /></div><p className="muted">负责人：{node.owner_employee_no || task.main_assignee_employee_no || "未指定"} · {node.status} · {node.progress_percent}%</p><p><strong>验收：</strong>{node.acceptance_criteria || "未设置"}</p><div className="action-row">{(nodeActions.get(node.node_id) || []).filter(isNodeExecutionAction).map((action) => <button className="button secondary" disabled={mutation.isPending} type="button" key={action} onClick={() => mutation.mutate({ action, nodeId: node.node_id })}>{actionLabels[action]}</button>)}</div></div></article>)}</div></section>
